...
async def addsk(sk, pk):
    try:
        find_sk = BLACKLISTED_SKS.find_one({"id": sk}, {"_id": 0})

        if str(find_sk) != "None":
            return  # SK is blacklisted, no need to check further

        if "test" in sk or "pk_" in sk:
            return  # SK is invalid, no need to check further

        all_sks = await getallsk()

        for i in all_sks:
            if i["id"] == sk:
                return  # SK already exists, no need to insert
...
async def getsk():
    ...
    sks_with_pk = await getallsk()
    try:
        sk_and_pk = random.choice(sks_with_pk)
        return sk_and_pk["id"], sk_and_pk.get("public_key", "")
    except:
        return "sk_live_MTCTECHX007X", ""  # Default values if database is empty or encounters an error
...
async def getallsk():
    ...
    for i in find:
        sks_with_pk.append({"id": i["id"], "public_key": i.get("public_key", "")})

    if len(sks_with_pk) == 0:
        return [{"id": "sk_live_MTCTECHX007X", "public_key": ""}]
    else:
        return sks_with_pk
...
async def get_random_info(session):
    try:
        ...
        add2        = str(data['results'][0]['location']['street']['number']) + ' ' + data['results'][0]['location']['street']['name']
