...
async def massfunc(fullcc , user_id , bearer_token ):
    try:
...
Bot by - <a href="tg://user?id=5371579102">[ 🇵🇰 ] ＳＹΞＤ 👑</a>
        </b> """
...
"""
        ...
        # proxies      = await get_proxy_format() 
        # session      = httpx.AsyncClient(timeout = 100 , proxies = proxies ) 
        works        = [massfunc(i, user_id, bearer_token ) for i in ccs]
