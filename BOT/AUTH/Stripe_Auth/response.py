...
async def get_auth_response(result, user_id, fullcc):
    try:
        ...
        if type(result) == str:
        ...
        elif '"seller_message": "Payment complete."' in result.text or "succeeded" in result.text:
            status   = "Live 🟢"
            ...
            hits = "NO"
            await result_logs(fullcc, "Stripe Auth", result)
