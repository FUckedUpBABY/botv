...
def analyze_url_and_forward(_, message):
    try:
        ...
        if len(command_parts) < 2:
            message.reply_text('**Please provide at least one Url**.')
            return
        ...
        if not urls:
            message.reply_text('**Please Provide at least one valid URL**.')
            return

        if len(urls) > 30:
            message.reply_text('**You can Analyze up to 30 URLs at a time**')
            return

        checking_msg = message.reply_text('**Checking URLs...**')
...
def format_result(result):
    formatted_result = (
        f"<b>┏━━━━━━━━━ {result['url']} Results ━━━━━━━━━┓</b>\n"
        f"<b>┃ ➔ Gateways:</b> <code>{', '.join(result['payment_gateways']) if result['payment_gateways'] else 'None'}</code>\n"
        f"<b>┃ ✧ Captcha:</b> <code>{result['captcha']}</code>\n"
        f"<b>┃ 🌐 Cloudflare:</b> <code>{result['cloudflare']}</code>\n"
        f"<b>┃ ✦ GraphQL:</b> <code>{result['graphql']}</code>\n"
        f"<b>┃ ➞ Platform:</b> <code>{result['platform']}</code>\n"
        f"<b>┃ ☠ Error:</b> <code>{result['error']}</code>\n"
        f"<b>┗━ Status:</b> <code>{result['http_status']}</code>\n"
        f"<i>━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━</i>\n"
    )
